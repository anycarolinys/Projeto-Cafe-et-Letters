...
public class Estoque implements ArquivosProdutos {
    ...
    public void excluirProduto(String codigo) {
        if (!excluirProdutoTexto(new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosTxt"), codigo)) {
           // excluirProdutoJson(new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosJson"), codigo);
        }
    }
    ...
    public void inicializarEstoque() {
        ...
        File produtosEmTxt = new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosTxt");
        ...
        File produtosJson = new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosJson");
    ...
    public void instanciarLivroObjeto(Livro livro, boolean pasta) {
        ...
        if (pasta == true)
            diretorioProdutos = new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosTxt");
        else
            diretorioProdutos = new File("./src/main/java/model/gestaoProdutos/produtosExcluidos");
...
//    }
    ...
    public List<String> listarProdutosExcluidos() {
        ...
        File arquivosExcluidos = new File("./src/main/java/model/gestaoProdutos/produtosExcluidos");
    ...
    public List<String> listarProdutosForaEstoque() {
        List<String> nomeProdutos = new ArrayList<>();
        File arquivosForaEstoque = new File("./src/main/java/model/gestaoProdutos/produtosForaEstoque");
    ...
    public List<String> listarAcompanhamentos() {
        ...
        for (Produto produto : getProdutos()) {
            if (produto.getClass().getSimpleName().equalsIgnoreCase("Acompanhamento")) {
    ...
    public void aumentarEstoque(String codigo, int aumento) {
        if(!aumentarEstoqueTexto(codigo, aumento)) {
        //    aumentarEstoqueJson(codigo, aumento);
        }
    }

    private void alterarEstoqueTexto(String caminhoArquivo, int aumento) {

        try {
            Path path = Paths.get(caminhoArquivo);
            List<String> linhas = Files.readAllLines(path);
            int qtdEmEstoque = Integer.parseInt(linhas.get(2));
            qtdEmEstoque += aumento;
    ...
    private boolean aumentarEstoqueTexto(String codigo, int aumento) {
        Produto produto = buscarProduto(codigo);
        

        File diretorioProdutos = new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosTxt");
        List<String> arquivosProdutos = new ArrayList<>();

        percorrerArquivosEmPasta(diretorioProdutos, arquivosProdutos);

        for (String arquivo : arquivosProdutos) {
            File arquivoProduto = new File(diretorioProdutos.getPath() + "/" + arquivo);
            try (
                FileReader leitor = new FileReader(arquivoProduto);
                BufferedReader buffer = new BufferedReader(leitor);
            ) {
                buffer.readLine();
                String codigoProduto = buffer.readLine();
                
                if (codigoProduto.equals(codigo)) {
                    alterarEstoqueTexto(arquivoProduto.getPath(), aumento);
                    int qtdEmEstoque = produto.getQtdEmEstoque() + aumento;
                    produto.setQtdEmEstoque(qtdEmEstoque);
                    return true;
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private void aumentarEstoqueJson(String codigo, int aumento) {
        File pastaJson = new File("./src/main/java/model/gestaoProdutos/produtosCadastrados/produtosJson");
        ...
        for (String arquivo : arquivosDeProdutos) {
            ...
            if (temProdutoJson(array, produto.getCodigo(), tipoProduto)) {
                alterarEstoqueJson(array, produto.getCodigo(), aumento, tipoProduto);
                int qtdEmEstoque = produto.getQtdEmEstoque() + aumento;
                produto.setQtdEmEstoque(qtdEmEstoque);
            }

            try (FileWriter writer = new FileWriter(arquivoProduto); PrintWriter printer = new PrintWriter(writer);) {
                printer.print(array.toString());
            } catch (IOException e) {
                e.printStackTrace();
            }
